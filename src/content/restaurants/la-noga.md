---
name: 'La Noga'
altNames:
  - 'La Noga Taverna'
tagline: 'Hort propi, trinxat de referència i menú per menys de setze euros'
town: 'Bellver de Cerdanya'
subregion: 'Baixa Cerdanya'
country: 'ES'
address: 'Pujada de Joan Alay, 2, 25720 Bellver de Cerdanya'
lat: 42.3702
lng: 1.7745
locationNote: 'El punt és el centre de Bellver de Cerdanya: d’aquesta casa no en consta l’adreça exacta al mapa.'
instagram: 'https://www.instagram.com/lanogataverna/'
cuisine:
  - 'Catalana tradicional'
  - 'Slow Food / km 0'
occasion:
  - 'Dinar en família'
  - 'De pas'
priceRange: '€'
priceFrom: 16
rating: 4.9
ratingSource: 'Google'
services: []
signatureDishes:
  - 'Trinxat de la Cerdanya'
  - 'Ànec amb naps'
dataThin: true
sources:
  - label: 'Top Cerdanya — guia de restaurants'
    url: 'https://topcerdanya.com/guies/restaurants-cerdanya'
  - label: 'Instagram oficial'
    url: 'https://www.instagram.com/lanogataverna/'
images:
  - src: '../../assets/restaurants/la-noga/01.jpg'
    alt: 'Bellver de Cerdaña Lérida 06'
    author: 'María Sacristán'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Bellver_de_Cerda%C3%B1a_L%C3%A9rida_(1)_06.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/la-noga/02.jpg'
    alt: 'Bellver de Cerdaña Lérida 09'
    author: 'María Sacristán'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Bellver_de_Cerda%C3%B1a_L%C3%A9rida_(1)_09.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/la-noga/03.jpg'
    alt: 'Bellver de Cerdaña Lérida 12'
    author: 'María Sacristán'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Bellver_de_Cerda%C3%B1a_L%C3%A9rida_(1)_12.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/la-noga/04.jpg'
    alt: 'Bellver de Cerdaña Lérida 14'
    author: 'María Sacristán'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Bellver_de_Cerda%C3%B1a_L%C3%A9rida_(1)_14.jpg'
    isOfficial: false
---

Cuina catalana de temporada amb una particularitat que aquí compta molt: la verdura ve de l'hort
que tenen ells. El trinxat és el plat pel qual se'ls cita, i l'ànec amb naps és l'altre clàssic
cerdà que hi trobareu. El menú comença per sota dels setze euros i la valoració és de les més
altes de la comarca, cosa que en aquesta franja de preu no és freqüent.

Avís d'honestedat: no tenen web i l'Instagram no es deixa consultar automàticament, així que no
publiquem telèfon ni horaris. Els números que circulen per directoris automàtics no els hem pogut
verificar en cap font fiable i hem preferit no posar-los.

## Què demanar

El trinxat. És literalment allò pel qual la resta de la comarca els coneix.
