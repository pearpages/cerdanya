---
name: 'Ca l’Eudald'
tagline: 'Des del 1972, el lloc on la cuina cerdana no s’ha mogut de lloc'
town: 'Alp'
subregion: 'Baixa Cerdanya'
country: 'ES'
address: 'Passeig Agnès Fabra, 4, 17538 Alp'
lat: 42.375233
lng: 1.889876
phone: '+34 972 890 033'
email: 'hotelrestaurant@caleudald.com'
website: 'https://www.caleudald.com/'
cuisine:
  - 'Catalana tradicional'
  - 'Slow Food / km 0'
occasion:
  - 'Dinar en família'
  - 'Après-ski'
priceRange: '€€'
priceFrom: 11
priceTo: 27
services: []
signatureDishes:
  - 'Trinxat de la Cerdanya'
  - 'Escudella'
  - 'Arròs de muntanya'
  - 'Trumfes confitades amb botifarra'
  - 'Magret amb salsa de genciana'
  - 'Peus de porc amb naps de la Cerdanya'
featured: true
featuredRank: 12
sources:
  - label: 'Web oficial'
    url: 'https://www.caleudald.com/'
  - label: 'Mengem — Ara'
    url: 'https://mengem.ara.cat/restaurants/ca-l-eudald-reivindicant-producte-cerdanya_1_4830975.html'
  - label: 'Viure als Pirineus'
    url: 'https://www.viurealspirineus.cat/articulo/els-fogons-del-pirineu/ca-leudald-dalp-cuina-tradicional-cerdanya-amb-producte-local-i-carta-temporada/20221102212609035885.html'
images:
  - src: '../../assets/restaurants/ca-l-eudald/01.jpg'
    alt: 'Exterior de Ca l’Eudald'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/02.jpg'
    alt: 'Exterior de Ca l’Eudald, 2'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/03.jpg'
    alt: 'Exterior de Ca l’Eudald, 3'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/04.jpg'
    alt: 'Exterior de Ca l’Eudald, 4'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/05.jpg'
    alt: 'Ca l’Eudald, a Alp'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/06.jpg'
    alt: 'Exterior de Ca l’Eudald, 5'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/07.jpg'
    alt: 'Ca l’Eudald, a Alp, 2'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
  - src: '../../assets/restaurants/ca-l-eudald/08.jpg'
    alt: 'Exterior de Ca l’Eudald, 6'
    author: 'Ca l’Eudald'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.caleudald.com/'
    isOfficial: true
---

El 1972 l'Eudald Vila i la Paquita Pons van obrir a Alp l'Aerohotel Cerdanya. Mig segle després la
casa es diu Ca l'Eudald i la porten els fills, l'Eudald i en Frank, amb l'Adriana Gubert. El que hi
fan és el receptari de la comarca sense retocs innecessaris: trinxat, escudella, arròs de muntanya,
trumfes confitades amb botifarra, cargols, bolets amb crema, peus de porc amb naps de la Cerdanya i
un magret amb salsa de genciana que és tan d'aquí com la resta. Sala de fusta i bigues vistes, i una
carta de vins llarga. És la referència de cuina tradicional d'Alp i no ho amaga.

## Què demanar

El trinxat, sempre. I els peus de porc amb naps, que és el plat que costa més de trobar bé fet en
qualsevol altra banda.
