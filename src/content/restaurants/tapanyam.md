---
name: 'TapaNyam'
altNames:
  - 'El Mirador TapaNyam'
tagline: 'Una barra llarga de tapes i una terrassa que mira a les teulades i al Pirineu'
town: 'Puigcerdà'
subregion: 'Baixa Cerdanya'
country: 'ES'
address: 'Plaça de l’Alguer, 2, 17520 Puigcerdà'
phone: '+34 972 88 23 60'
email: 'restauranttapanyam@hotmail.com'
instagram: 'https://www.instagram.com/elmiradortapanyam/'
cuisine:
  - 'Tapes'
  - 'Muntanya i brasa'
  - 'Peix i marisc'
occasion:
  - 'Terrassa d’estiu'
  - 'De pas'
  - 'Dinar en família'
priceRange: '€€'
rating: 4.5
ratingSource: 'Google'
services:
  - 'terrassa'
  - 'grups'
signatureDishes:
  - 'Cargols a la llauna'
  - 'Foies'
  - 'Trinxat'
  - 'Entrecot amb salsa de pebre verd'
dataThin: true
sources:
  - label: 'Girona Restaurants'
    url: 'https://gironarestaurants.cat/item/puigcerda-restaurant-tapanyam/'
  - label: 'Instagram oficial'
    url: 'https://www.instagram.com/elmiradortapanyam/'
images:
  - src: '../../assets/restaurants/tapanyam/01.jpg'
    alt: 'Estació de Puigcerdà 1'
    author: 'marlène delhaye'
    license: 'CC BY 2.0'
    licenseUrl: 'https://creativecommons.org/licenses/by/2.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Estaci%C3%B3_de_Puigcerd%C3%A0_1.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/tapanyam/02.jpg'
    alt: 'Plaça dels Herois, Puigcerdà'
    author: 'Mariarosafg'
    license: 'CC BY-SA 3.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Pla%C3%A7a_dels_Herois,_Puigcerd%C3%A0.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/tapanyam/03.jpg'
    alt: 'Puigcerdà, a la Cerdanya'
    author: 'Pere López Brosa'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Puigcerd%C3%A0_-_P1100352.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/tapanyam/04.jpg'
    alt: 'Plaça Cabrinetty'
    author: 'Pere López'
    license: 'CC BY-SA 3.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Pla%C3%A7a_Cabrinetty_P1030206.JPG'
    isOfficial: false
---

El sobrenom que fan servir a Instagram, «el mirador», explica per què s'hi va: la terrassa dona a
les teulades de pissarra de Puigcerdà i, al darrere, als prats que pugen cap al Pirineu. A dins hi
ha una barra llarga amb les tapes a la vista i un menjador gran, de vuitanta places, que aguanta
celebracions. La carta és àmplia —tapes, peix i marisc, carn a la brasa, i fins i tot pizzes
artesanes— i la casa marca com a especialitat els foies i els cargols a la llauna. Compte: el
domini tapanyam.es ja no és seu i avui serveix una altra cosa; el canal viu és l'Instagram.

## Què demanar

Els cargols a la llauna, que és el plat que ells mateixos posen al davant. I taula a la terrassa,
si fa dia.
