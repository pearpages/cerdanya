---
name: 'El Bistrot de Llívia'
tagline: 'Català, cerdà i francès en una sala petita plena de raquetes de neu'
town: 'Llívia'
subregion: 'Llívia'
country: 'ES'
address: 'Avinguda Catalunya, 58, 17527 Llívia'
cuisine:
  - 'Bistrot'
  - 'Catalana tradicional'
  - 'Francesa'
occasion:
  - 'Dinar en família'
  - 'Après-ski'
priceRange: '€€'
priceFrom: 20
priceTo: 30
services: []
signatureDishes: []
sources:
  - label: 'Gastroranking'
    url: 'https://gastroranking.es/r/el-bistrot-de-llivia_152840/'
  - label: 'Amigastronómicas'
    url: 'https://www.amigastronomicas.com/2024/02/28/el-bistrot-de-llivia-comida-cerdanya/'
images:
  - src: '../../assets/restaurants/el-bistrot-de-llivia/01.jpg'
    alt: 'Llívia, a la Cerdanya'
    author: 'Pere López Brosa'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Ll%C3%ADvia_-_P1100377.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/el-bistrot-de-llivia/02.jpg'
    alt: 'Llívia, a la Cerdanya, 2'
    author: 'Pere López Brosa'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Ll%C3%ADvia_-_P1100378.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/el-bistrot-de-llivia/03.jpg'
    alt: 'Farmacia Esteva'
    author: 'Alberto-g-rovi'
    license: 'CC BY 3.0'
    licenseUrl: 'https://creativecommons.org/licenses/by/3.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Farmacia_Esteva_(1).jpg'
    isOfficial: false
  - src: '../../assets/restaurants/el-bistrot-de-llivia/04.jpg'
    alt: 'Farmacia Esteva, 2'
    author: 'Alberto-g-rovi'
    license: 'CC BY 3.0'
    licenseUrl: 'https://creativecommons.org/licenses/by/3.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Farmacia_Esteva_(3).jpg'
    isOfficial: false
---

Llívia és espanyola i està envoltada de França per tots costats, i aquesta carta ho reflecteix
sense fer-ne teoria: hi conviuen el receptari català, el cerdà i el francès en una quarantena de
plats. La sala és petita i la decoració va de muntanya de debò —raquetes de neu, esquís vells
penjats—, cosa que en un altre lloc seria disfressa i aquí és simplement el que hi ha al garatge.
Es menja bé per vint o trenta euros. S'omple sempre, així que val més trucar amb un parell de dies.

## Què demanar

La carta és llarga i canvia; el criteri més segur és demanar el que sigui de temporada i deixar-se
recomanar.
