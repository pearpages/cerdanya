---
name: 'Més Que Beure'
tagline: 'Va començar sent un lloc per beure i ha acabat sent un lloc per menjar'
town: 'Llívia'
subregion: 'Llívia'
country: 'ES'
address: 'Carrer Frederic Bernades, 27, 17527 Llívia'
lat: 42.464337
lng: 1.982189
phone: '+34 692 96 20 36'
instagram: 'https://www.instagram.com/mesquebeure/'
cuisine:
  - 'Tapes'
  - 'Cuina de mercat'
occasion:
  - 'Dinar en família'
  - 'De pas'
priceRange: '€€'
priceFrom: 30
priceTo: 40
rating: 4.8
reviewCount: 412
ratingSource: 'Restaurant Guru'
services: []
signatureDishes:
  - 'Xurros bravos'
  - 'Cigrons amb sípia i gambes'
  - 'Steak tartar'
  - 'Croquetes casolanes'
  - 'Raviolis amb bolets'
hours: 'De dijous a diumenge, servei continu de 12:30 a 22:00'
closedDays: 'Dilluns, dimarts i dimecres'
sources:
  - label: 'Instagram oficial'
    url: 'https://www.instagram.com/mesquebeure/'
  - label: 'Restaurant Guru'
    url: 'https://restaurantguru.com/Mes-Que-Beure-Llivia'
images:
  - src: '../../assets/restaurants/mes-que-beure/01.jpg'
    alt: 'Llívia plaça'
    author: 'No machine-readable author provided. MarkCat~commonswiki assumed (based on copyright claims).'
    license: 'CC BY-SA 3.0'
    licenseUrl: 'http://creativecommons.org/licenses/by-sa/3.0/'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Ll%C3%ADvia_pla%C3%A7a.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/mes-que-beure/02.jpg'
    alt: 'Llívia Plaça Major en 1994'
    author: 'Jean-Pierre Bazard'
    license: 'CC BY-SA 3.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/3.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Ll%C3%ADvia_-_Pla%C3%A7a_Major_en_1994.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/mes-que-beure/03.jpg'
    alt: 'Llivia, place'
    author: 'Jiròni B.'
    license: 'CC BY-SA 4.0'
    licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Llivia,_place.jpg'
    isOfficial: false
  - src: '../../assets/restaurants/mes-que-beure/04.jpg'
    alt: 'Vista interior de l''església de Llívia'
    author: 'Josep Salvany i Blanch'
    license: 'Public domain'
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Vista_interior_de_l%27esgl%C3%A9sia_de_Ll%C3%ADvia.jpeg'
    isOfficial: false
---

El nom era una promesa modesta —més que beure— i se'ls ha quedat curt: el que va néixer com un
lloc de vins a Llívia s'ha convertit en una cuina amb prou ambició per aguantar un àpat sencer.
Van de tapes catalanes amb alguna incursió de fusió, i el format és flexible: hi pots anar a fer
un vermut llarg o a dinar de debò, perquè fan servei continu de dotze i mitja a deu del vespre. Els
porta una parella, l'Emília i en David. És dels més ben valorats de Llívia, que en un poble amb
aquesta concentració de restaurants vol dir alguna cosa.

## Què demanar

Els xurros bravos i els cigrons amb sípia i gambes. Amb un parell de coses més ja no cal dinar
enlloc.
