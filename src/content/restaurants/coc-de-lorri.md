---
name: 'Coc de l’Orri'
tagline: 'La cuina de la cabana de pastor, a l’antic estable de la casa'
town: 'Lles de Cerdanya'
subregion: 'Baixa Cerdanya'
country: 'ES'
address: 'Carrer dels Escolls, 5, 25726 Lles de Cerdanya'
website: 'https://www.cocdelorri.cat/'
cuisine:
  - 'Catalana tradicional'
  - 'Cuina de mercat'
occasion:
  - 'Dinar en família'
  - 'Sopar especial'
priceRange: '€€'
services: []
signatureDishes: []
hours: 'De dimecres a diumenge al migdia, 13:00–15:30. Divendres i dissabte, també al vespre.'
closedDays: 'Dilluns i dimarts'
sources:
  - label: 'Pirineus TV'
    url: 'https://pirineustv.cat/lles-perd-la-historica-fonda-domingo-pero-conserva-la-familia-que-la-gestionava'
  - label: 'Pirineus Digital'
    url: 'https://pirineusdigital.cat/lles-perd-la-historica-fonda-domingo-pero-conserva-la-familia-que-la-gestionava/'
images:
  - src: '../../assets/restaurants/coc-de-lorri/01.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/02.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya, 2'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/03.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya, 3'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/04.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya, 4'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/05.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya, 5'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/06.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya, 6'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/07.jpg'
    alt: 'Sala de Coc de l’Orri'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
  - src: '../../assets/restaurants/coc-de-lorri/08.jpg'
    alt: 'Coc de l’Orri, a Lles de Cerdanya, 7'
    author: 'Coc de l’Orri'
    license: 'Cortesia del restaurant'
    sourceUrl: 'https://www.cocdelorri.cat/'
    isOfficial: true
---

Un orri és una cabana de pedra de pastor, d'aquelles on es feia nit quan el bestiar pujava d'un
orri a l'altre; als Pirineus hi ha una Canal de l'Orri i un Pic de l'Orri. D'aquí el nom i la idea:
la cuina de la cabana. El porten en Joan Campmajó, als fogons, i la Clara Parés, a la sala, que
durant dotze anys i mig havien portat la històrica Fonda Domingo de Lles fins que se'ls va acabar
el contracte; en comptes de marxar del poble, van obrir casa seva. L'edifici era l'estable de les
vaques, el galliner i la cort dels porcs, i l'han reformat per dins mantenint-ne l'estructura.

## Què demanar

No publiquen carta en obert i el seu web no admet consultes automatitzades, així que la
recomanació honesta és trucar i preguntar què hi ha aquella setmana.
